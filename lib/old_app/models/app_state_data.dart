...
import '../../new_app/models/drive_energy_report.dart';
import '../../new_app/models/drive_integration_model.dart';
...
class AppStateData extends ChangeNotifier {
  ...
  // NEW: Drive integration properties
  DriveIntegrationConfig? _driveConfig;
  bool _isDriveEnabled = false;
  List<DriveEnergyReport> _pendingReports = [];
  DriveConnectionStatus _driveStatus = DriveConnectionStatus.pending;

  // NEW: Drive getters
  DriveIntegrationConfig? get driveConfig => _driveConfig;
  bool get isDriveEnabled => _isDriveEnabled;
  List<DriveEnergyReport> get pendingReports =>
      List.unmodifiable(_pendingReports);
  DriveConnectionStatus get driveStatus => _driveStatus;

  // NEW: Load drive configuration
  Future<void> loadDriveConfig() async {
    if (_currentUser?.subdivisionId == null) return;

    try {
      final configDoc = await FirebaseFirestore.instance
          .collection('drive_configs')
          .doc(_currentUser!.subdivisionId)
          .get();

      if (configDoc.exists) {
        _driveConfig = DriveIntegrationConfig.fromFirestore(configDoc);
        _isDriveEnabled = _driveConfig!.autoSyncEnabled;
        _driveStatus = _driveConfig!.connectionStatus;
      }
    } catch (e) {
      print('ERROR: Failed to load drive config: $e');
    }

    notifyListeners();
  }

  // NEW: Update drive configuration
  Future<void> updateDriveConfig(DriveIntegrationConfig config) async {
    try {
      await FirebaseFirestore.instance
          .collection('drive_configs')
          .doc(config.substationId)
          .set(config.toFirestore());

      _driveConfig = config;
      _isDriveEnabled = config.autoSyncEnabled;
      _driveStatus = config.connectionStatus;

      notifyListeners();
    } catch (e) {
      print('ERROR: Failed to update drive config: $e');
      throw e;
    }
  }

  // NEW: Queue report for drive upload
  void queueReportForDrive(DriveEnergyReport report) {
    _pendingReports.add(report);
    notifyListeners();

    // Trigger upload if auto-sync is enabled
    if (_isDriveEnabled) {
      _uploadPendingReports();
    }
  }

  // NEW: Upload pending reports
  Future<void> _uploadPendingReports() async {
    if (!_isDriveEnabled || _driveConfig == null) return;

    for (final report in List.from(_pendingReports)) {
      try {
        // Here you would implement the actual Google Drive upload
        // This is a placeholder for the drive service call
        await _uploadReportToDrive(report);

        _pendingReports.remove(report);
      } catch (e) {
        print('ERROR: Failed to upload report ${report.id}: $e');
        // Update report with error status
        final updatedReport = report.copyWith(
          uploadStatus: DriveUploadStatus.failed,
          errorMessage: e.toString(),
          retryCount: report.retryCount + 1,
        );

        final index = _pendingReports.indexOf(report);
        if (index != -1) {
          _pendingReports[index] = updatedReport;
        }
      }
    }
  ...
  // NEW: Placeholder for actual drive upload
  Future<void> _uploadReportToDrive(DriveEnergyReport report) async {
    // This would contain the actual Google Drive API calls
    // For now, just simulate success after a delay
    await Future.delayed(Duration(seconds: 2));

    // Update report status
    final updatedReport = report.copyWith(
      uploadStatus: DriveUploadStatus.completed,
      uploadedAt: Timestamp.now(),
      driveFileId: 'simulated_file_id_${DateTime.now().millisecondsSinceEpoch}',
      driveFileUrl: 'https://drive.google.com/file/d/simulated_file_id',
    );

    // Save to Firestore
    await _saveReportToFirestore(updatedReport);
  }

  // NEW: Save report to Firestore
  Future<void> _saveReportToFirestore(DriveEnergyReport report) async {
    try {
      await FirebaseFirestore.instance
          .collection('drive_reports')
          .doc(report.id)
          .set(report.toFirestore());
    } catch (e) {
      print('ERROR: Failed to save report to Firestore: $e');
    }
  }
