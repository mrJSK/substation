...
//   final List<Bay>?
//   allSubstationBays; // ✅ NEW: All bays in substation for substation config
//   final Map<String, BusbarEnergyMap>? currentConfiguration;
//   final Map<String, BusbarEnergyMap>?
//   substationConfiguration; // ✅ NEW: Substation config
//   final Function(Map<String, BusbarEnergyMap>, Map<String, BusbarEnergyMap>?)
...
//     this.allSubstationBays, // ✅ NEW: Optional
//     this.currentConfiguration,
//     this.substationConfiguration, // ✅ NEW: Optional
...
//   // ✅ NEW: Added mixin for tabs

//   // ✅ NEW: Tab controller
//   late TabController _tabController;

//   // Existing busbar configuration
...
//     // ✅ NEW: Initialize tab controller (2 tabs if substation bays provided, 1 otherwise)
...
//     // Initialize busbar configuration
//     for (var bay in widget.connectedBays) {
//       final existingConfig =
//           widget.currentConfiguration?['${widget.busbar.id}-${bay.id}'];
...
//     // ✅ NEW: Initialize substation configuration
...
//         final existingConfig = widget
//             .substationConfiguration?['${widget.busbar.substationId}-${busbar.id}'];
...
//       // Create BusbarEnergyMap for each connected bay
//       final Map<String, BusbarEnergyMap> busbarConfigMap = {};

//       for (var bay in widget.connectedBays) {
//         final mapKey = '${widget.busbar.id}-${bay.id}';
//         busbarConfigMap[mapKey] = BusbarEnergyMap(
//           id: mapKey,
//           substationId: widget.busbar.substationId ?? '',
//           busbarId: widget.busbar.id,
//           connectedBayId: bay.id,
//           importContribution:
...
//       // ✅ NEW: Create substation configuration if provided
//       Map<String, BusbarEnergyMap>? substationConfigMap;
...
//         for (var busbar in allBusbars) {
//           final mapKey = '${widget.busbar.substationId}-${busbar.id}';
//           substationConfigMap[mapKey] = BusbarEnergyMap(
//             id: mapKey,
//             substationId: widget.busbar.substationId ?? '',
//             busbarId:
//                 'SUBSTATION', // Special identifier for substation-level mapping
//             connectedBayId: busbar.id,
//             importContribution:
//                 _substationImpSelectionMap[busbar.id] ??
//                 EnergyContributionType.busImport,
//             exportContribution:
...
//       widget.onSaveConfiguration(busbarConfigMap, substationConfigMap);
...
//             ),
//           ),
//           // ✅ NEW: Add TabBar if substation configuration is available
...
//             ? TabBarView(
//                 // ✅ NEW: TabBarView for multiple tabs
//                 controller: _tabController,
//                 children: [
...
//                 ],
//               )
//             : _buildBusbarTab(isDarkMode), // Single tab content
...
//   // ✅ NEW: Busbar tab content (extracted from original body)
...
//   // ✅ NEW: Substation busbar configuration card
...
//   // ✅ NEW: Substation energy config dropdown
...
//   // ✅ NEW: Empty state for substation tab
...
class BusbarConfigurationScreen extends StatefulWidget {
  ...
  final List<Bay> connectedBays;
  final Map<String, BusbarEnergyMap>? currentConfiguration;
  final Function(Map<String, BusbarEnergyMap>) onSaveConfiguration;
...
class _BusbarConfigurationScreenState extends State<BusbarConfigurationScreen> {
  ...
  void initState() {
    ...
    // Initialize with current configuration or defaults
    for (var bay in widget.connectedBays) {
      final existingConfig =
          widget.currentConfiguration?['${widget.busbar.id}-${bay.id}'];
      _bayImpSelectionMap[bay.id] =
  ...
  Future<void> _saveConfiguration() async {
    ...
    try {
      ...
      // Create BusbarEnergyMap for each bay
      final Map<String, BusbarEnergyMap> configMap = {};

      for (var bay in widget.connectedBays) {
        final mapKey = '${widget.busbar.id}-${bay.id}';
        configMap[mapKey] = BusbarEnergyMap(
          id: mapKey, // Will be updated with Firestore document ID
          substationId: widget.busbar.substationId ?? '',
          busbarId: widget.busbar.id,
          connectedBayId: bay.id,
          importContribution:
          ...
          exportContribution:
              _bayExpSelectionMap[bay.id] ?? EnergyContributionType.none,
          lastModified: DateTime.now(),
          modifiedBy: 'current_user_id', // This should come from the parent
        );
      }
